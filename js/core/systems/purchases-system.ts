...
export function purchaseStraw({ sips, straws, cups, widerStraws, betterCups }: { sips: number; straws: number; cups: number; widerStraws: number; betterCups: number }) {
  ...
  const { strawSPD, cupSPD, sipsPerDrink } = recalcProduction({ straws: newStraws, cups, widerStraws, betterCups });
  return { spent: cost, straws: newStraws, strawSPD, cupSPD, sipsPerDrink };
}

export function purchaseCup({ sips, straws, cups, widerStraws, betterCups }: { sips: number; straws: number; cups: number; widerStraws: number; betterCups: number }) {
  const { upgrades, config } = getTypedConfig();
  ...
  const { strawSPD, cupSPD, sipsPerDrink } = recalcProduction({ straws, cups: newCups, widerStraws, betterCups });
  return { spent: cost, cups: newCups, strawSPD, cupSPD, sipsPerDrink };
}

export function purchaseWiderStraws({ sips, straws, cups, widerStraws, betterCups }: { sips: number; straws: number; cups: number; widerStraws: number; betterCups: number }) {
  const { upgrades, config } = getTypedConfig();
  ...
  const { strawSPD, cupSPD, sipsPerDrink } = recalcProduction({ straws, cups, widerStraws: newWiderStraws, betterCups });
  return { spent: cost, widerStraws: newWiderStraws, strawSPD, cupSPD, sipsPerDrink };
}

export function purchaseBetterCups({ sips, straws, cups, widerStraws, betterCups }: { sips: number; straws: number; cups: number; widerStraws: number; betterCups: number }) {
  const { upgrades, config } = getTypedConfig();
  ...
  const { strawSPD, cupSPD, sipsPerDrink } = recalcProduction({ straws, cups, widerStraws, betterCups: newBetterCups });
...
export function upgradeSuction({ sips, suctionUpCounter }: { sips: number; suctionUpCounter: number }) {
...
export function purchaseFasterDrinks({ sips, fasterDrinks }: { sips: number; fasterDrinks: number }) {
  const { upgrades: up, config } = getTypedConfig();
...
export function upgradeFasterDrinks({ sips, fasterDrinksUpCounter }: { sips: number; fasterDrinksUpCounter: number }) {
  const { upgrades: up, config } = getTypedConfig();
...
export function purchaseCriticalClick({ sips, criticalClicks, criticalClickChance }: { sips: number; criticalClicks: number; criticalClickChance: number }) {
  const { upgrades: up, config } = getTypedConfig();
...
export function upgradeCriticalClick({ sips, criticalClickUpCounter, criticalClickMultiplier }: { sips: number; criticalClickUpCounter: number; criticalClickMultiplier: number }) {
  const { config } = getTypedConfig();
...
export function levelUp({ sips, level, sipsPerDrink }: { sips: number; level: number; sipsPerDrink: number }) {
  const { config } = getTypedConfig();
