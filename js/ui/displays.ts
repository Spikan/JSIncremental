...
export function updateDrinkProgress(progress?: number, drinkRate?: number): void {
    ...
    const progressFill = (window as any).DOM_CACHE?.progressFill || document.getElementById('drinkProgressFill');
    const countdown = (window as any).DOM_CACHE?.countdown || document.getElementById('drinkCountdown');
...
export function updateTopSipCounter(): void {
    if (typeof window === 'undefined') return;
    const topSipElement = (window as any).DOM_CACHE?.topSipValue || document.getElementById('topSipValue');
    if (topSipElement) {
        try {
