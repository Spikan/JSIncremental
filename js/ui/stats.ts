...
export function updateLastSaveTime(): void {
    ...
    try {
        const lastSaveMs = Number((window as any).App?.state?.getState?.()?.lastSaveTime || (window as any).lastSaveTime || 0);
...
export function updateClickStats(): void {
    ...
    // Critical clicks
    const criticalClicksElement = (window as any).DOM_CACHE?.criticalClicksStats as HTMLElement | undefined;
    ...
    // Best click streak
    const bestClickStreakElement = (window as any).DOM_CACHE?.bestClickStreak as HTMLElement | undefined;
...
export function updateEconomyStats(): void {
    // Total sips earned
    const totalSipsEarnedElement = (window as any).DOM_CACHE?.totalSipsEarned as HTMLElement | undefined;
    ...
    const highestSipsPerSecondElement = (window as any).DOM_CACHE?.highestSipsPerSecond as HTMLElement | undefined;
...
export function updateAchievementStats(): void {
    ...
    // Faster drinks owned
    const fasterDrinksOwnedElement = (window as any).DOM_CACHE?.fasterDrinksOwned as HTMLElement | undefined;
