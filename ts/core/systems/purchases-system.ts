...
export function purchaseFriends({
  sips,
  friends,
}: {
  sips: number | DecimalType;
  friends: number | DecimalType;
}) {
  const { upgrades: up, config } = getTypedConfig();
  const baseCost = up?.friends?.baseCost ?? config.FRIENDS_BASE_COST;
  const scaling = up?.friends?.scaling ?? config.FRIENDS_SCALING;

  // Convert inputs to Decimal for calculations
  const sipsLarge = toDecimal(sips);
  const friendsLarge = toDecimal(friends);

  const baseCostLarge = new Decimal(baseCost);
  const scalingLarge = new Decimal(scaling);
  // Use direct Decimal pow for extreme values instead of toSafeNumber
  const cost = baseCostLarge.multiply(scalingLarge.pow(friendsLarge));

  // Check affordability using Decimal comparison
  if (!gte(sipsLarge, cost)) return null;

  const newFriends = friendsLarge.add(new Decimal(1));
...
export function upgradeFriends({
  ...
  sips: number | DecimalType;
  friendsUpCounter: number | DecimalType;
}) {
  const { upgrades: up, config } = getTypedConfig();
  const base = up?.friends?.upgradeBaseCost ?? config.FRIENDS_UPGRADE_BASE_COST ?? 0;

  // Convert inputs to Decimal for calculations
  const sipsLarge = toDecimal(sips);
  const counterLarge = toDecimal(friendsUpCounter);
...
export const execute = {
  ...
  buyFriends(): boolean {
    ...
    try {
      ...
      console.warn('Failed to update sips after friends purchase:', error);
    }
    try {
      w.friends = new (w.Decimal || Number)(
        (result.friends as any).toString?.() ?? String(result.friends)
      );
    } catch (error) {
      console.warn('Failed to update friends after purchase:', error);
    }
    try {
      const actions = w.App?.state?.actions;
      actions?.setSips?.(w.sips);
      actions?.setFriends?.(result.friends);
    } catch (error) {
      console.warn('Failed to update App.state via actions after friends purchase:', error);
    }
    try {
      w.App?.state?.setState?.({
        sips: w.sips,
      ...
      console.warn('Failed to fallback setState after friends purchase:', error);
    }
    try {
      w.App?.ui?.checkUpgradeAffordability?.();
    } catch (error) {
      console.warn('Failed to check upgrade affordability after friends purchase:', error);
    }
    try {
      w.App?.ui?.updateShopButtonStates?.();
    } catch (error) {
      console.warn('Failed to update shop button states after friends purchase:', error);
    }
    try {
      w.App?.ui?.updateAllStats?.();
    } catch (error) {
      console.warn('Failed to update all stats after friends purchase:', error);
    }
    try {
      w.App?.ui?.updateShopStats?.();
      w.App?.ui?.updateAllDisplays?.();
    } catch (error) {
      console.warn('Failed to update shop displays after friends purchase:', error);
    }
    try {
      w.App?.events?.emit?.(w.App?.EVENT_NAMES?.ECONOMY?.PURCHASE, {
        item: 'friends',
      ...
      console.warn('Failed to emit purchase event for friends:', error);
    }
    return true;
  },
  upgradeFriends(): boolean {
    const st = getAppState();
    const result = upgradeFriends({
      sips: st.sips,
      friendsUpCounter: st.friendsUpCounter,
    });
    if (!result) return false;
    const w: any = (typeof window !== 'undefined' ? window : {}) as any;
    try {
      ...
      console.warn('Failed to update sips after friends upgrade:', error);
    }
    try {
      const actions = w.App?.state?.actions;
      actions?.setSips?.(w.sips);
      actions?.setFriendsUpCounter?.(result.friendsUpCounter);
    } catch (error) {
      console.warn('Failed to update App.state via actions after friends upgrade:', error);
    }
    try {
      w.App?.state?.setState?.({
        sips: w.sips,
        friendsUpCounter: result.friendsUpCounter,
      });
    } catch (error) {
      console.warn('Failed to fallback setState after friends upgrade:', error);
    }
    try {
      w.App?.ui?.checkUpgradeAffordability?.();
    } catch (error) {
      console.warn('Failed to check upgrade affordability after friends upgrade:', error);
    }
    try {
      w.App?.ui?.updateShopButtonStates?.();
    } catch (error) {
      console.warn('Failed to update shop button states after friends upgrade:', error);
    }
    try {
      w.App?.ui?.updateAllStats?.();
    } catch (error) {
      console.warn('Failed to update all stats after friends upgrade:', error);
    }
    try {
      w.App?.ui?.updateShopStats?.();
      w.App?.ui?.updateAllDisplays?.();
    } catch (error) {
      console.warn('Failed to update shop displays after friends upgrade:', error);
    }
    try {
      w.App?.events?.emit?.(w.App?.EVENT_NAMES?.ECONOMY?.PURCHASE, {
        item: 'friendsUpgrade',
      ...
      console.warn('Failed to emit purchase event for friends upgrade:', error);
    }
    return true;
  },
};
