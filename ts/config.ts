// Typed config export that mirrors previous JS structure

import { errorHandler } from './core/error-handling/error-handler';

export const config = {
  GAME_VERSION: '1.0.0',
  GAME_TITLE: 'Soda Clicker Pro!',
  UI: {
    FOUNTAIN_SODA_PROGRESS: true,
    SODA_BUTTON_PROGRESS: true,
    ENABLE_3D_SODA_BUTTON: false,
    USE_THREE_SODA_BUTTON: false,
    USE_PROCEDURAL_CUP: true,
    EXPERIMENTAL_FLUID: false,
    EXPERIMENTAL_FLUID_SURFACE: false,
    STREAM_RIBBON_NOISE: true,
    STREAM_IMPACT_GLINT: true,
    SURFACE_HIGHLIGHT: true,
  },
  BALANCE: {
    STRAW_BASE_COST: 15,
    STRAW_SCALING: 1.1,
    CUP_BASE_COST: 60,
    CUP_SCALING: 1.12,
    SUCTION_BASE_COST: 10,
    SUCTION_SCALING: 1.08,
    FASTER_DRINKS_BASE_COST: 45,
    FASTER_DRINKS_SCALING: 1.08,
    CRITICAL_CLICK_BASE_COST: 45,
    CRITICAL_CLICK_SCALING: 1.1,
    WIDER_STRAWS_BASE_COST: 120,
    WIDER_STRAWS_SCALING: 1.1,
    BETTER_CUPS_BASE_COST: 275,
    BETTER_CUPS_SCALING: 1.12,
    LEVEL_UP_BASE_COST: 3000,
    LEVEL_UP_SCALING: 1.15,
    SUCTION_UPGRADE_BASE_COST: 800,
    FASTER_DRINKS_UPGRADE_BASE_COST: 1500,
    CRITICAL_CLICK_UPGRADE_BASE_COST: 1200,
    STRAW_BASE_SPD: 2.5,
    CUP_BASE_SPD: 6.0,
    SUCTION_CLICK_BONUS: 1.0,
    BASE_SIPS_PER_DRINK: 1,
    WIDER_STRAWS_MULTIPLIER: 0.5,
    BETTER_CUPS_MULTIPLIER: 0.4,
    CRITICAL_CLICK_BASE_CHANCE: 0.01,
    CRITICAL_CLICK_CHANCE_INCREMENT: 0.001,
    CRITICAL_CLICK_BASE_MULTIPLIER: 10,
    CRITICAL_CLICK_MULTIPLIER_INCREMENT: 5,
    LEVEL_UP_SIPS_MULTIPLIER: 1.5,
    FASTER_DRINKS_REDUCTION_PER_LEVEL: 0.015,
    FASTER_DRINKS_UPGRADE_BONUS_PER_LEVEL: 0.002,
    MIN_DRINK_RATE: 500,
  },
  TIMING: {
    DEFAULT_DRINK_RATE: 4200,
    MIN_SAVE_INTERVAL: 1000,
    AUTOSAVE_INTERVAL: 10,
    OFFLINE_MIN_TIME: 30,
    OFFLINE_MAX_TIME: 3600,
    CLICK_STREAK_WINDOW: 1500,
    CLICK_FEEDBACK_DURATION: 1000,
    CRITICAL_FEEDBACK_DURATION: 2000,
    LEVEL_UP_FEEDBACK_DURATION: 800,
    PURCHASE_FEEDBACK_DURATION: 2000,
    UNLOCK_NOTIFICATION_DURATION: 3000,
    SODA_CLICK_ANIMATION_DURATION: 150,
    SPLASH_TRANSITION_DELAY: 500,
    MUSIC_RETRY_DELAY: 2000,
    DOM_READY_DELAY: 100,
    MUSIC_INIT_DELAY: 1000,
  },
  LIMITS: {
    MAX_CLICK_TIMES: 100,
    MAX_MUSIC_RETRIES: 10,
    TARGET_FPS: 60,
    STATS_UPDATE_INTERVAL: 1000,
    AFFORDABILITY_CHECK_INTERVAL: 200,
    CLICK_FEEDBACK_RANGE_X: 80,
    CLICK_FEEDBACK_RANGE_Y: 40,
    PROGRESS_BAR_NEARLY_COMPLETE: 75,
    PROGRESS_BAR_COMPLETE: 100,
  },
  AUDIO: {
    SPLASH_MUSIC_VOLUME: 0.15,
    MAIN_MUSIC_VOLUME: 0.1,
    CLICK_SOUND_BASE_FREQ_MIN: 200,
    CLICK_SOUND_BASE_FREQ_MAX: 300,
    CLICK_SOUND_DURATION_MIN: 0.1,
    CLICK_SOUND_DURATION_MAX: 0.2,
    CLICK_SOUND_VOLUME_MIN: 0.3,
    CLICK_SOUND_VOLUME_MAX: 0.5,
    CRITICAL_CLICK_FREQ1_MIN: 150,
    CRITICAL_CLICK_FREQ1_MAX: 230,
    CRITICAL_CLICK_FREQ2_MIN: 300,
    CRITICAL_CLICK_FREQ2_MAX: 420,
    CRITICAL_CLICK_DURATION_MIN: 0.08,
    CRITICAL_CLICK_DURATION_MAX: 0.2,
    CRITICAL_CLICK_VOLUME_MIN: 0.25,
    CRITICAL_CLICK_VOLUME_MAX: 0.4,
    LEVEL_UP_FREQ1_MIN: 180,
    LEVEL_UP_FREQ1_MAX: 240,
    LEVEL_UP_FREQ2_MIN: 350,
    LEVEL_UP_FREQ2_MAX: 450,
    LEVEL_UP_FREQ3_MIN: 500,
    LEVEL_UP_FREQ3_MAX: 650,
    LEVEL_UP_DURATION_MIN: 0.12,
    LEVEL_UP_DURATION_MAX: 0.2,
    LEVEL_UP_VOLUME_MIN: 0.2,
    LEVEL_UP_VOLUME_MAX: 0.35,
    PURCHASE_FREQ1_MIN: 400,
    PURCHASE_FREQ1_MAX: 600,
    PURCHASE_FREQ2_MIN: 800,
    PURCHASE_FREQ2_MAX: 1100,
    PURCHASE_FREQ3_MIN: 1200,
    PURCHASE_FREQ3_MAX: 1600,
    PURCHASE_DURATION_MIN: 0.3,
    PURCHASE_DURATION_MAX: 0.5,
    PURCHASE_VOLUME_MIN: 0.4,
    PURCHASE_VOLUME_MAX: 0.6,
    SODA_CLICK_FREQ1_MIN: 80,
    SODA_CLICK_FREQ1_MAX: 120,
    SODA_CLICK_FREQ2_MIN: 160,
    SODA_CLICK_FREQ2_MAX: 220,
    SODA_CLICK_DURATION_MIN: 0.4,
    SODA_CLICK_DURATION_MAX: 0.6,
    SODA_CLICK_VOLUME_MIN: 0.35,
    SODA_CLICK_VOLUME_MAX: 0.5,
  },
  UNLOCKS: {
    // Legacy threshold-based unlocks (kept for backward compatibility)
    SUCTION: { sips: 8, clicks: 3 },
    CRITICAL_CLICK: { sips: 30, clicks: 8 },
    FASTER_DRINKS: { sips: 45, clicks: 10 },
    STRAWS: { sips: 18, clicks: 6 },
    CUPS: { sips: 60, clicks: 18 },
    WIDER_STRAWS: { sips: 140, clicks: 24 },
    BETTER_CUPS: { sips: 275, clicks: 36 },
    LEVEL_UP: { sips: 800, clicks: 60 },
    SHOP: { sips: 60, clicks: 10 },
    STATS: { sips: 200, clicks: 20 },
    GOD: { sips: 5000, clicks: 300 },
    UNLOCKS_TAB: { sips: 10, clicks: 5 },
  },
  // New unlock purchase costs (one-time purchases)
  UNLOCK_PURCHASES: {
    SUCTION: 15,
    CRITICAL_CLICK: 45,
    FASTER_DRINKS: 45,
    STRAWS: 25,
    CUPS: 60,
    WIDER_STRAWS: 140,
    BETTER_CUPS: 275,
    LEVEL_UP: 1200,
    SHOP: 60,
    STATS: 180,
    GOD: 15000,
    UNLOCKS_TAB: 25,
  },
  FORMATTING: {
    SMALL_NUMBER_THRESHOLD: 1000,
    MEDIUM_NUMBER_THRESHOLD: 1e6,
    LARGE_NUMBER_THRESHOLD: 1e9,
    HUGE_NUMBER_THRESHOLD: 1e12,
    MASSIVE_NUMBER_THRESHOLD: 1e15,
    ENORMOUS_NUMBER_THRESHOLD: 1e18,
    GIGANTIC_NUMBER_THRESHOLD: 1e21,
    TITANIC_NUMBER_THRESHOLD: 1e24,
    COSMIC_NUMBER_THRESHOLD: 1e27,
    INFINITE_NUMBER_THRESHOLD: 1e30,
    DECIMAL_PLACES_SMALL: 2,
    DECIMAL_PLACES_MEDIUM: 1,
    DECIMAL_PLACES_LARGE: 2,
  },
  MOBILE: {
    MAX_WIDTH: 768,
    MAX_HEIGHT: 1024,
    USER_AGENT_PATTERNS: [
      'Android',
      'webOS',
      'iPhone',
      'iPad',
      'iPod',
      'BlackBerry',
      'IEMobile',
      'Opera Mini',
    ],
  },
  PERFORMANCE: {
    LOW_FPS_WARNING_THRESHOLD: 30,
    FRAME_COUNT_INTERVAL: 1000,
  },
} as const;

// Mirror to window for non-module consumers (legacy)
try {
  (window as any).GAME_CONFIG = config;
} catch (error) {
  errorHandler.handleError(error, 'exposeGameConfigGlobally');
}

export type GameConfig = typeof config;
